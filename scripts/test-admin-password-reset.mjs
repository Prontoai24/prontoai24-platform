...
try {
  ...
  const profile = await admin.from('profiles').upsert({ id: userId, role: 'admin', email, full_name: 'Admin Reset E2E', status: 'active', must_change_password: true }, { onConflict: 'id' })
  ...
  const completed = await admin.from('profiles').update({ status: 'active', must_change_password: false, updated_at: new Date().toISOString() }).eq('id', userId)
  ...
  const checkedProfile = await admin.from('profiles').select('role, status, must_change_password').eq('id', userId).single()
  if (checkedProfile.error || checkedProfile.data?.must_change_password !== false || checkedProfile.data?.status !== 'active' || checkedProfile.data?.role !== 'admin') throw new Error(`Profilo non completato: ${JSON.stringify(checkedProfile.data)}`)
  ...
  console.log(JSON.stringify({ ok: true, email, passwordSpecialCharactersAccepted: '=@$!%*#?&', profile: checkedProfile.data, authMetadata: { must_change_password: false, force_password_change: false }, secondLogin: true, adminDashboardEligible: true }, null, 2))
} finally {
