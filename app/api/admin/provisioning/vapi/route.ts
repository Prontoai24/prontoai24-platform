...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error: saveError } = await adminClient.from('phone_numbers').upsert({ ...(phoneNumberId ? { id: phoneNumberId } : {}), client_id: clientId, phone_number: phoneNumber, vapi_phone_id: importedPhone.id || importedPhone.phoneNumberId, telnyx_number_id: importedPhone.telnyxPhoneNumberId || null, status: 'active' }, { onConflict: phoneNumberId ? 'id' : 'id' })
