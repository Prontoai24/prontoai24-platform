...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (profileReadError) return jsonError(`Impossibile leggere il profilo Admin: ${profileReadError.message}`, 500)
    ...
    if (profileUpdateError) return jsonError(`Password aggiornata, ma profiles non è stato aggiornato: ${profileUpdateError.message}`, 500)
    const profile = updatedProfile || (await adminClient.from('profiles').upsert({ id: user.id, ...profileValues }, { onConflict: 'id' }).select('id, role, email, status, must_change_password').single()).data
    if (!profile || profile.must_change_password !== false || profile.status !== 'active') return jsonError('Password aggiornata, ma il profilo non è stato completato correttamente.', 500)
